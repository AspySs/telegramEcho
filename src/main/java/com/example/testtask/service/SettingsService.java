...
public class SettingsService {
    ...
    @Autowired
    public SettingsService(SettingsRepository settingsRepository){this.repository = settingsRepository;}


    public Settings findSettingsByName(String name){
        if(name.isEmpty()){throw new IllegalArgumentException("Settings name is empty!");}
        Optional<Settings> sett = repository.findByName(name);
        if(sett.isEmpty()){throw new SettingsNotFoundException("Settings not found by name");}
        return sett.get();
    }
